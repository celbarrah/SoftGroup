...
const OUVRAGES = [
  {
    num: "01", title: "Un Bouquet de Proverbes Marocains",
  ...
  {
    num: "02", title: "1 001 Manières de Porter Châle et Foulard",
  ...
  {
    num: "03", title: "Le Maghreb des Origines à Nos Jours : Vision 2050",
  ...
  {
    num: "04", title: "Contes et Proverbes Marocains",
    desc: "Un voyage illustré au coeur de l'imaginaire marocain, célébrant contes, symboles et traditions populaires.",
  ...
  {
    num: "05", title: "Coeur de Mécènes, Mécènes de Coeur",
  ...
  return (
    ...
    >
      <div
        ...
        style={{ position: "relative", cursor: "pointer", borderRadius: 2, overflow: "hidden", transformStyle: "preserve-3d", transition: "transform 0.35s cubic-bezier(0.22,1,0.36,1)", willChange: "transform", aspectRatio: "3 / 4.2" }}
      >
        <img src={book.image} alt={book.title} loading="lazy"
          style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", transition: "transform 0.7s cubic-bezier(0.22,1,0.36,1)", transform: hovered ? "scale(1.08)" : "scale(1.0)" }} />

        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.25) 40%, rgba(0,0,0,0.0) 70%)" }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(10,8,6,0.97) 0%, rgba(10,8,6,0.72) 55%, rgba(10,8,6,0.15) 85%, transparent 100%)", opacity: hovered ? 1 : 0, transition: "opacity 0.55s ease" }} />
        <div style={{ position: "absolute", top: 0, left: "-100%", width: "60%", height: "100%", background: "linear-gradient(to right, transparent, rgba(196,165,90,0.07), transparent)", transform: hovered ? "translateX(300%)" : "translateX(0%)", transition: "transform 0.8s ease", pointerEvents: "none" }} />

        <div style={{ position: "absolute", top: 16, left: 16, fontFamily: "var(--font-dm-sans, sans-serif)", fontSize: 9, letterSpacing: "0.4em", textTransform: "uppercase", color: "rgba(196,165,90,0.65)", fontWeight: 500 }}>
          {book.num}
        </div>

        <div style={{ position: "absolute", bottom: 0, left: 0, height: 2, background: "#C4A55A", width: hovered ? "100%" : "24px", transition: "width 0.55s cubic-bezier(0.22,1,0.36,1)" }} />

        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 20px 24px", transform: hovered ? "translateY(0)" : "translateY(18px)", opacity: hovered ? 1 : 0, transition: "transform 0.5s cubic-bezier(0.22,1,0.36,1), opacity 0.45s ease" }}>
          <h3 style={{ fontFamily: "var(--font-cormorant, serif)", fontSize: "clamp(1rem, 1.2vw, 1.2rem)", fontWeight: 300, color: "#ffffff", lineHeight: 1.3, margin: "0 0 10px" }}>{book.title}</h3>
          <p style={{ fontFamily: "var(--font-dm-sans, sans-serif)", fontSize: 11, color: "rgba(255,255,255,0.55)", lineHeight: 1.65, margin: "0 0 8px" }}>{book.desc}</p>
          {book.author && (
            <p style={{ fontFamily: "var(--font-dm-sans, sans-serif)", fontSize: 9, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(196,165,90,0.75)", margin: 0 }}>{book.author}</p>
          )}
        </div>

        <div style={{ position: "absolute", bottom: 14, left: 20, right: 20, opacity: hovered ? 0 : 1, transform: hovered ? "translateY(6px)" : "translateY(0)", transition: "opacity 0.35s ease, transform 0.4s ease" }}>
          <h3 style={{ fontFamily: "var(--font-cormorant, serif)", fontSize: "clamp(0.9rem, 1.05vw, 1.05rem)", fontWeight: 300, color: "#fff", lineHeight: 1.3, margin: 0 }}>{book.title}</h3>
        </div>
      </div>
    </motion.div>
  )
...
export default function PoleActivite() {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }}>
            ...
            <div className="flex items-center gap-5 mb-2">
              <img src="https://res.cloudinary.com/dofyrwzop/image/upload/q_auto/f_auto/v1779210148/CULTURE_qffzuo-removebg-preview_pkud3g.png"
                alt="SoftCulture" style={{ height: 56, width: "auto", objectFit: "contain", flexShrink: 0 }} />
              <h2 className="font-serif text-3xl md:text-5xl text-neutral-800 font-light leading-[1.1]">SoftCulture</h2>
            </div>
